# FIWARE Dataspace MVD on Kubernetes

## Introduction 

In order to support development and exploration of the FIWARE Data Space Connector, a "Minimal Viable Dataspace" is
provided as part of this repo. Other deployment methods are provided:
- A deployment guide, based on [MicroK8s](https://microk8s.io/) can be found here: https://github.com/cgonzalezITA/DSFiware-hackathon?tab=readme-ov-file#quick-deployment-from-scratch

## Requirements

Since the local deployment installs 2 connector instances and a trust-anchor, its recommended to use a Kubernetes cluster with: 
...
Prior to deploying the Dataspace, the ArgoCD application must be modified to set:
- A domain name for the consumer (e.g. dataconsumer.fr)
- A domain name for the procucer (e.g  dataprovider.fr)
- A domain name for the trust-ancher (e.g trust-anchor.fr)
...
```shell
    kubectl -n argocd apply -f deploy/argocd/dataspace/fiware-dataspace.yaml
```
...
The deployed Data Space consists of 2 Participants, connected through a Trust Anchor.
...
The admin-console of keycloak is available at: ```https://keycloak.dataconsumer.fr```, login with
the `keycloak-admin`
The credentials issuance in the account-console is available
at: ```https://keycloak.dataconsumer.fr/realms/test-realm/account/oid4vci```, login with the `test-user`
...
```shell
  export ACCESS_TOKEN=$(curl -s -k  -X POST https://keycloak.dataconsumer.fr/realms/test-realm/protocol/openid-connect/token \
    --header 'Accept: */*' \
...
```shell
  curl -k  -X GET https://keycloak.dataconsumer.fr/realms/test-realm/.well-known/openid-credential-issuer | jq .
```

Get a credential offer uri(for the `user-credential), using the retrieved AccessToken:

```shell
  export OFFER_URI=$(curl -s -k  -X GET 'https://keycloak.dataconsumer.fr/realms/test-realm/protocol/oid4vc/credential-offer-uri?credential_configuration_id=user-credential' \
...
```shell
  export CREDENTIAL_ACCESS_TOKEN=$(curl -s -k  -X POST https://keycloak.dataconsumer.fr/realms/test-realm/protocol/openid-connect/token \
    --header 'Accept: */*' \
    --header 'Content-Type: application/x-www-form-urlencoded' \
...
```shell
  export VERIFIABLE_CREDENTIAL=$(curl -s -k  -X POST https://keycloak.dataconsumer.fr/realms/test-realm/protocol/oid4vc/credential \
    --header 'Accept: */*' \
    --header 'Content-Type: application/json' \
...
```shell
  curl -s -X 'POST' http://pap-provider.dataprovider.fr/policy \
...
an ingress at ```http://scorpio-provider.dataprovider.fr/ngsi-ld/v1```. In
...
```shell
  curl -s -X POST http://scorpio-provider.dataprovider.fr/ngsi-ld/v1/entities \
...
```shell
  curl -s -X GET 'http://mp-data-service.dataprovider.fr/ngsi-ld/v1/entities/urn:ngsi-ld:EnergyReport:fms-1'
```

The normal flow is now to request the oidc-information at the well-known endpoint:

```shell
  export TOKEN_ENDPOINT=$(curl -s -X GET 'http://mp-data-service.dataprovider.fr/.well-known/openid-configuration' | jq -r '.token_endpoint'); echo $TOKEN_ENDPOINT
```
...
```json
{
  "issuer": "https://provider-verifier.dataprovider.fr",
  "authorization_endpoint": "https://provider-verifier.dataprovider.fr",
  "token_endpoint": "https://provider-verifier.dataprovider.fr/services/data-service/token",
  "jwks_uri": "https://provider-verifier.dataprovider.fr/.well-known/jwks",
  "scopes_supported": [
    "default"
  ],
...
With that information, the authentication flow at the verifier(e.g.`"https://provider-verifier.dataprovider.fr`)
can be started.
...
2. Create the product specification. The specification should contain the configuration of credentials to be trusted by the customer(in this case, "OperatorCredential" with the role "OPERATOR" for the target-service) and the policy to be created for the customer. In the example, we will offer 2 different types service: "small" and "full", where small restricts the cluster-size to be created:
  ...
  1. "small": 
  ```shell
  export PRODUCT_SPEC_SMALL_ID=$(curl -X 'POST' http://tm-forum-api.dataprovider.fr/tmf-api/productCatalogManagement/v4/productSpecification \
  ...
  1. "full": 
  ```shell
  export PRODUCT_SPEC_FULL_ID=$(curl -X 'POST' http://tm-forum-api.dataprovider.fr/tmf-api/productCatalogManagement/v4/productSpecification \
    -H 'Content-Type: application/json;charset=utf-8' \
    -d "{
      \"brand\": \"M&P Operations\",
      \"version\": \"1.0.0\",
...
3. Create a product offering, referencing the spec:
  ...
  1. "small":
  ```shell
  export PRODUCT_OFFERING_SMALL_ID=$(curl -X 'POST' http://tm-forum-api.dataprovider.fr/tmf-api/productCatalogManagement/v4/productOffering \
    -H 'Content-Type: application/json;charset=utf-8' \
    -d "{
      \"version\": \"1.0.0\",
      \"lifecycleStatus\": \"ACTIVE\",
  ...
  2. "small":
  ```shell
  export PRODUCT_OFFERING_FULL_ID=$(curl -X 'POST' http://tm-forum-api.dataprovider.fr/tmf-api/productCatalogManagement/v4/productOffering \
    -H 'Content-Type: application/json;charset=utf-8' \
    -d "{
      \"version\": \"1.0.0\",
      \"lifecycleStatus\": \"ACTIVE\",
...
```shell
export USER_CREDENTIAL=$(./doc/scripts/get_credential.sh https://keycloak.dataconsumer.fr user-credential employee); echo ${USER_CREDENTIAL}
```
2. Get the LegalPersonCredential for the representative:
```shell
export REP_CREDENTIAL=$(./doc/scripts/get_credential.sh https://keycloak.dataconsumer.fr user-credential representative); echo ${REP_CREDENTIAL}
```
3. Get the OperatorCredential for the operator:
```shell
export OPERATOR_CREDENTIAL=$(./doc/scripts/get_credential.sh https://keycloak.dataconsumer.fr operator-credential operator); echo ${OPERATOR_CREDENTIAL}
```
...
```shell
  export ACCESS_TOKEN=$(./doc/scripts/get_access_token_oid4vp.sh http://mp-data-service.dataprovider.fr $USER_CREDENTIAL default); echo ${ACCESS_TOKEN}
  curl -X POST http://mp-data-service.dataprovider.fr/ngsi-ld/v1/entities \
    -H 'Accept: */*' \
    -H 'Content-Type: application/json' \
...
```shell
    ./doc/scripts/get_access_token_oid4vp.sh http://mp-data-service.dataprovider.fr $OPERATOR_CREDENTIAL operator
```

> :bulb: For easier access, the identity(in form of the did) of the consumer is available at ```http://did-consumer.dataprovider.fr/did-material/did.env```.
...
```shell
  export ACCESS_TOKEN=$(./doc/scripts/get_access_token_oid4vp.sh http://mp-data-service.dataconsumer.fr $REP_CREDENTIAL default); echo ${ACCESS_TOKEN}
  export FANCY_MARKETPLACE_ID=$(curl -X POST http://mp-tmf-api.dataconsumer.fr/tmf-api/party/v4/organization \
    -H 'Accept: */*' \
    -H 'Content-Type: application/json' \
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
    -d "{
...
with the TMForum-APIs directly.
  1. List offerings
  ```shell
    export ACCESS_TOKEN=$(./doc/scripts/get_access_token_oid4vp.sh http://mp-data-service.dataprovider.fr $REP_CREDENTIAL default); echo $ACCESS_TOKEN
...
```shell
  export ACCESS_TOKEN=$(./doc/scripts/get_access_token_oid4vp.sh http://mp-data-service.dataprovider.fr $OPERATOR_CREDENTIAL operator); echo $ACCESS_TOKEN 
  curl -X POST http://mp-data-service.dataprovider.fr/ngsi-ld/v1/entities \
    -H 'Accept: */*' \
    -H 'Content-Type: application/json' \
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
...
```shell
  export ACCESS_TOKEN=$(./doc/scripts/get_access_token_oid4vp.sh http://mp-data-service.dataprovider.fr $OPERATOR_CREDENTIAL operator)
  curl -X GET http://mp-data-service.dataprovider.fr/ngsi-ld/v1/entities/urn:ngsi-ld:K8SCluster:fancy-marketplace \
    -H 'Accept: */*' \
    -H "Authorization: Bearer ${ACCESS_TOKEN}"
```
...
```shell
  ./doc/scripts/get_access_token_oid4vp.sh http://mp-data-service.dataprovider.fr $USER_CREDENTIAL default
```
...
Get one via:
```shell
 export ELSI_CREDENTIAL=$(./doc/scripts/get_credential.sh http://keycloak.dataconsumer.fr:8080 user-credential); echo ${ELSI_CREDENTIAL}
```
...
|--------------------------------------------------|--------------------------------------------|----------------------------------------------------------------------------|--------------|-------------------------------------------------------------------------------------|------------------------------------------------------|
| http://tir.trust-anchor.fr/                | http://tir.127.0.0.1.nip.io/               | [Trusted Issuers Registry](https://github.com/FIWARE/trusted-issuers-list) | Trust Anchor | Provides the list of trusted issuers                                                | no                                                   |
| http://til.trust-anchor.fr/                | http://til.127.0.0.1.nip.io/               | [Trusted Issuers List API](https://github.com/FIWARE/trusted-issuers-list) | Trust Anchor | Create,Update,Delete functionality for the Trusted Issuers Registry                 | should not be publicly available in real data spaces |
| http://keycloak.dataconsumer.fr/  | https://keycloak.dataconsumer.fr | [Keycloak](https://github.com/keycloak/keycloak)                           | Consumer     | Issues credentials on behalf of the consumer                                        | no                                                   |
| https://marketplace.dataprovider.fr/       | https://marketplace.dataprovider.fr | [BAE Marketplace](https://github.com/FIWARE-TMForum/business-ecosystem-logic-proxy)                    | Provider     | Provide marketplace functionality                                                   | no                                                   |
| http://keycloak-provider.dataprovider.fr/  | https://keycloak-provider.dataprovider.fr | [Keycloak](https://github.com/keycloak/keycloak)                           | Provider     | Issues credentials on behalf of the provider                                        | no                                                   |
| http://did-consumer.dataprovider.fr/       | http://did-consumer.dataprovider.fr/      | [did-helper](https://github.com/wistefan/did-helper)                       | Consumer     | Helper to provide access to the consumers did and key material                      | yes, should never be public                          |
| http://mp-data-service.dataprovider.fr/    | http://mp-data-service.dataprovider.fr/   | [Apisix](https://apisix.apache.org/)                                       | Provider     | ApiGateway to be used as entry point to all secured services, f.e. the data-service | no                                                   |
| https://provider-verifier.dataprovider.fr/ | https://provider-verifier.io               | [VCVerifier](https://github.com/FIWARE/VCVerifier)                         | Provider     | Authentication endpoint, used for authenticating through VerifiableCredentials      | no                                                   |
| http://did-provider.dataprovider.fr/       | http://did-provider.dataprovider.fr/      | [did-helper](https://github.com/wistefan/did-helper)                       | Provider     | Helper to provide access to the providers did and key material                      | yes, should never be public                          |
| http://scorpio-provider.dataprovider.fr/   | http://scorpio-provider.dataprovider.fr/  | [Scorpio ContextBroker](https://github.com/ScorpioBroker/ScorpioBroker)    | Provider     | Provides direct access to the context broker, to be used for test setup.            | yes, should only be available through the PEP        |
| http://pap-provider.dataprovider.fr/       | http://pap-provider.dataprovider.fr       | [ODRL-PAP](https://github.com/wistefan/odrl-pap)                           | Provider     | Allows configuration of the access policie, used for authorization                  | yes, should only be available to authorized users    |
...
The deployment leverages  Argocd. 
